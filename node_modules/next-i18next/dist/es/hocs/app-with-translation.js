...
export const appWithTranslation = function (WrappedComponent) {
  ...
  const clientLoadNamespaces = (lng, namespaces) => Promise.all(namespaces.filter(ns => !i18n.hasResourceBundle(lng, ns)).map(ns => i18n.reloadResources(lng, ns)));
  ...
  class AppWithTranslation extends React.Component {
      ...
      if (!isServer()) {
        ...
        i18n.changeLanguage = async (newLng, callback = () => null) => {
          ...
          if (typeof newLng === 'string' && i18n.initializedLanguageOnce === true) {
            const usedNamespaces = Object.entries(i18n.reportNamespaces.usedNamespaces).filter(x => x[1] === true).map(x => x[0]);
    ...
    render() {
      ...
      }, /*#__PURE__*/React.createElement(NextStaticProvider, null, /*#__PURE__*/React.createElement(WrappedComponentWithSSR, _extends({

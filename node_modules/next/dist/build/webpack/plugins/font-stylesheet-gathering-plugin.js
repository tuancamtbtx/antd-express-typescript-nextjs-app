"use strict";exports.__esModule=true;exports.FontStylesheetGatheringPlugin=void 0;var _webpack=_interopRequireDefault(require("webpack"));var _webpackSources=_interopRequireDefault(require("webpack-sources"));var _fontUtils=require("../../../next-server/server/font-utils");var _postcss=_interopRequireDefault(require("postcss"));var _cssnanoSimple=_interopRequireDefault(require("cssnano-simple"));var _constants=require("../../../next-server/lib/constants");function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}// @ts-ignore: TODO: remove ignore when webpack 5 is stable
const{RawSource}=_webpack.default.sources||_webpackSources.default;const isWebpack5=parseInt(_webpack.default.version)===5;let BasicEvaluatedExpression;if(isWebpack5){BasicEvaluatedExpression=require('webpack/lib/javascript/BasicEvaluatedExpression');}else{BasicEvaluatedExpression=require('webpack/lib/BasicEvaluatedExpression');}async function minifyCss(css){return new Promise(resolve=>(0,_postcss.default)([(0,_cssnanoSimple.default)({excludeAll:true,discardComments:true,normalizeWhitespace:{exclude:false}})]).process(css,{from:undefined}).then(res=>{resolve(res.css);}));}class FontStylesheetGatheringPlugin{constructor(){this.compiler=void 0;this.gatheredStylesheets=[];this.manifestContent=[];this.parserHandler=factory=>{const JS_TYPES=['auto','esm','dynamic'];// Do an extra walk per module and add interested visitors to the walk.
...
if(parser==null?void 0:(_parser$state=parser.state)==null?void 0:(_parser$state$module=_parser$state.module)==null?void 0:_parser$state$module.resource.includes('node_modules')){return;}let result;if(node.name==='__jsx'){result=new BasicEvaluatedExpression();// @ts-ignore
result.setRange(node.range);result.setExpression(node);result.setIdentifier('__jsx');// This was added webpack 5.
if(isWebpack5){result.getMembers=()=>[];}}return result;});parser.hooks.call.for('__jsx').tap(this.constructor.name,node=>{if(node.arguments.length!==2){// A font link tag has only two arguments rel=stylesheet and href='...'
